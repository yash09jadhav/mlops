...
2. Create code.py and add code to it. (it will save a csv file to a new "employees" folder)

3. Do a git add-commit-push before initializing dvc.

4. "pip install dvc"
   "dvc init" (creates 1 file & 1 folder)

   .dvc/ → stores DVC configuration and metadata.
   .dvcignore → similar to .gitignore, tells DVC which files/folders to ignore.
